...
func NewInput(s string, isMultiLine bool) *Input {
	textArea := &Input{
		...
		TextBuilder: ui.NewMarkdownTxBuilder(),
...
func (i *Input) StartCapture() {
	...
	ui.Handle("/sys/kbd", func(e ui.Event) {
		if i.IsCapturing {
			...
			if i.Name == "" {
				ui.SendCustomEvt("/input/kbd", i.getInputEvt(key))
			} else {
				ui.SendCustomEvt("/input/"+i.Name+"/kbd", i.getInputEvt(key))
			}
...
func (i *Input) Buffer() ui.Buffer {
	...
	for n < len(cs) {
		...
		if x == 0 && i.ShowLineNo {
			...
			for lineNo := 0; lineNo < len(curLineNoRunes); lineNo++ {
				buf.Set(i.InnerX()+lineNo, i.InnerY()+y, curLineNoRunes[lineNo])
			}
			lineNoCnt++
		}
		...
		buf.Set(i.InnerX()+x+textXOffset, i.InnerY()+y, cs[n])
	...
	if lastLine > i.InnerBounds().Dy() {
		cursorYOffset += i.InnerBounds().Dy() - 1
	} else {
